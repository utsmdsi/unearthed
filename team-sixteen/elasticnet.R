...
set.seed(1123)
grinding.df <- grinding.df[sample(1:nrow(grinding.df),
                                  size = 10000,
                                  replace = FALSE), ]
...
# Variable importance
data.frame(predictor = row.names(importance(glmFit$finalModel)),
           MeanDecreaseGini = importance(glmFit$finalModel)) %>%
    arrange(desc(MeanDecreaseGini))

# == Save/Load the model == #
# Save
saveRDS(glmFit, "models/glmFit.rds")
# Load in model
# logreg <- readRDS("models/logregFit.rds")

# ==== Evaluate - testing set from caret ==== #
# Predict
glmPredict <- predict(glmFit, newdata = testing)
# Compute class probabilities for classification models
glmPredict_probs <- predict(glmFit, newdata = testing, type = "prob")

# Confusion matrix and assoicated statistics for the model fit
confusionMatrix(glmPredict, testing$surge)
# Two Class Summary with probs (for ROC)
twoClassSummary(data.frame(obs = testing$surge,
                           pred = glmPredict,
                           glmPredict_probs), lev = c("N","Y"))
